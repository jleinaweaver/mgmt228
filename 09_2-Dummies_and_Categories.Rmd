...
\item Dichotomous predictors (Dummies)
...
knitr::include_graphics('Images/09-2-Excel1.png')
```
...
2. Fit an OLS regression of income on the male dummy
...
Regress earnings (2021) on education and the male dummy
...
stargazer(res1, res2, res3, digits = 2, omit.stat = "rsq", star.cutoffs = .05, notes = "*p<0.05", notes.append = FALSE, dep.var.caption = "", dep.var.labels = "Income (Thousands USD)", header = FALSE, float = FALSE, font.size = "scriptsize")
```
\end{center}
...
\center
```{r, results = "asis"}
stargazer(res3, digits = 2, omit.stat = "rsq", star.cutoffs = .05, notes = "*p<0.05", notes.append = FALSE, dep.var.caption = "", dep.var.labels = "Income (Thousands USD)", covariate.labels = c("Education", "Male"), header = FALSE, float = FALSE, font.size = "scriptsize")
```
...
tibble(
    ...
    Male = round(as.numeric(x3[1] + x3[2]*Education + x3[3]*1), 1),
    Female = round(as.numeric(x3[1] + x3[2]*Education + x3[3]*0), 1)
) %>%
    stargazer(summary = FALSE, rownames = FALSE, digits = 2, header = FALSE, float = FALSE, font.size = "normalsize")
```
...
d %>%
  ...
  coord_cartesian(ylim = c(0, 200), xlim = c(9, 18)) +
...
```{r, fig.align = 'center', fig.asp=0.618, out.width = '100%', fig.width = 7}
knitr::include_graphics("./Images/09-1-Excel2.png")
```
