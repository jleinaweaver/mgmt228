...
Evaluating Multiple Linear Regressions (Wheelan ch12)
...
## YOU MADE HUGE CHANGES ON THE FLY, LET THE STUDENTS LEAD VARIABLE SELECTION
res1a <- d %>% lm(data = ., earnk2021 ~ education)
res1b <- d %>% lm(data = ., earnk2021 ~ angry)
res1c <- d %>% lm(data = ., earnk2021 ~ age)
res1d <- d %>% lm(data = ., earnk2021 ~ exercise)
res2 <- d %>% lm(data = ., earnk2021 ~ education + age + exercise)

summary(res2)

# baseline
predict.lm(res2, newdata = data.frame("education" = 13, "exercise" = 1, age = 20))

predict.lm(res2, newdata = data.frame("education" = 13, "exercise" = 7, age = 20))

predict.lm(res2, newdata = data.frame("education" = 16, "exercise" = 1, age = 22))

predict.lm(res2, newdata = data.frame("education" = 18, "exercise" = 1, age = 24))
...
FIRST, establish a baseline prediction for a hypothetical person who is 25 (Age), Education 13, Exercise 1, Height 67

\vspace{.2in}

SECOND, calculate the predicted income if that person:

\begin{enumerate}
\item Finishes college? (16)
\item Finishes grad school? (18)
\item Increases exercise to 1x / week? (4)
\item Increases exercise to 3x / week? (7)
\end{enumerate}
